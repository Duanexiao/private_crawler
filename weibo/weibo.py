...
class WeiBo(object):
    ...
    def validate_config(self, config):
        ...
        for argument in argument_list:
            if config[argument] != 0 and config[argument] != 1:
                sys.exit('{0}值应为0或1,请重新输入'.format(config[argument]))

        # 验证since_date
        since_date = str(config['since_date'])
        if (not self.is_date(since_date)) and (not since_date.isdigit()):
            sys.exit('since_date值应为yyyy-mm-dd形式或整数,请重新输入')
        ...
        for mode in config['write_mode']:
            if mode not in write_mode:
                sys.exit('{0}为无效模式，请从csv、json、mongo和mysql中挑选一个或多个作为write_mode'.format(mode))

        # 验证user_id_list
        user_id_list = config['user_id_list']
        if (not isinstance(user_id_list, list)) and (not user_id_list.endswith('.txt')):
            sys.exit('user_id_list值应为list类型或txt文件路径')
        if not isinstance(user_id_list, list):
            if not os.path.isabs(user_id_list):
                user_id_list = os.path.split(
                    os.path.realpath(__file__))[0] + os.sep + user_id_list
            if not os.path.isfile(user_id_list):
                sys.exit('不存在{0}文件'.format(user_id_list))
    ...
    def user_to_csv(self):
        """
            将爬取到的用户信息写入csv文件
        """
        file_dir = os.path.join(os.path.split(os.path.realpath(__file__))[0], 'weibo')
        if not os.path.isdir(file_dir):
            os.makedirs(file_dir)
        file_path = os.path.join(file_dir, 'users.csv')
        result_headers = [
    ...
    def user_to_mongodb(self):
        ...
        print('{0}信息写入MongoDB数据库完毕'.format(self.user['screen_name']))
    ...
    def user_to_mysql(self):
        ...
        self.mysql_create_database(self.mysql_config, create_database)
        # 创建'user'表
        create_table = """
        ...
        print('{0}信息写入MySQL数据库完毕'.format(self.user['screen_name']))
    ...
    def get_long_weibo(self, weibo_id):
        ...
        """
        for i in range(5):
            url = 'https://m.weibo.cn/detail/{0}'.format(weibo_id)
    ...
    def get_video_url(self, weibo_info):
        ...
        if weibo_info.get('page_info'):
            if weibo_info['page_info'].get('media_info') and weibo_info['page_info'].get('type') == 'video':
    ...
    def download_one_file(self, url, file_path, file_type, weibo_id):
        ...
        try:
            ...
            error_file = os.path.join(self.get_filepath(file_type), 'not_downloaded.txt')
    ...
    def download_files(self, file_type, weibo_type, wrote_count):
        ...
        try:
            ...
            print('即将进行{0}下载'.format(describe))
            ...
            print('{0}下载完毕,保存路径: '.format(describe))
            print(file_dir)
        except Exception as e:
            print('Error: ', repr(e))
            traceback.print_exc()
    ...
    def print_one_weibo(weibo):
        ...
        """
        try:
            print('微博id：{0}'.format(weibo['id']))
            print('微博正文：{0}'.format(weibo['text']))
            print('原始图片url：{0}'.format(weibo['pics']))
            print('微博位置：{0}'.format(weibo['location']))
            print('发布时间：{0}'.format(weibo['created_at']))
            print('发布工具：{0}'.format(weibo['source']))
            print('点赞数：{0}'.format(weibo['attitudes_count']))
            print('评论数：{0}'.format(weibo['comments_count']))
            print('转发数：{0}'.format(weibo['reposts_count']))
            print('话题：{0}'.format(weibo['topics']))
            print('@用户：{0}'.format(weibo['at_users']))
            print('url：https://m.weibo.cn/detail/{0}'.format(weibo['id']))
        except OSError:
            pass
                        ...
                        if wb:
                            ...
                            if created_at < since_date:
                                if self.is_pinned_weibo(w):
                                    ...
                                    print('{0}已获取{1}({2})的第{3}页微博{4}'.format(
                            ...
                            if (not self.filter) or (
                                ...
                                print('正在过滤转发微博')
            print('{0}已获取{1}({2})的第{3}页微博{4}'.format('-' * 30, self.user['screen_name'],
                                                     self.user['id'], page, '-' * 30))
        except Exception as e:
            print("Error: ", repr(e))
            traceback.print_exc()
    ...
    def get_filepath(self, file_type):
        ...
        """
        try:
            file_dir = os.path.join(os.path.split(os.path.realpath(__file__))[0], 'weibo', self.user['screen_name'])
            if file_type == 'img' or file_type == 'video':
                file_dir = os.path.join(file_dir, file_type)
            if not os.path.isdir(file_dir):
                os.makedirs(file_dir)
            if file_type == 'img' or file_type == 'video':
                return file_dir
            file_path = os.path.join(file_dir, self.user_config['user_id'] + '.' + file_type)
            return file_path
        except Exception as e:
            print('Error: ', repr(e))
            traceback.print_exc()
    ...
    def csv_helper(self, headers, result_data, file_path):
        ...
        if sys.version < '3':  # python2.x
            ...
            with open(file_path, 'a', encoding='utf-8-sig', newline='') as f:
                ...
                writer.writerows(result_data)
        if headers[0] == 'id':
            print('{0}条微博写入csv文件完毕,保存路径: {1}'.format(self.got_count, file_path))
        else:
            print('{0} 信息写入csv文件完毕，保存路径: {1}'.format(self.user['screen_name'], file_path))
    ...
    def write_json(self, wrote_count):
        ...
        print('{0}条微博写入json文件完毕,保存路径: {1}'.format(self.got_count, path))

    def info_to_mongodb(self, collection, info_list=None):
        """
            将爬取的信息写入MongoDB数据库
        """
        try:
            ...
            sys.exit('系统中可能没有安装pymongo库，请先运行 pip install pymongo ，再运行程序')
        try:
            ...
            sys.exit('系统中可能没有安装或启动MongoDB数据库，请先根据系统环境安装或启动MongoDB，再运行程序')
    ...
    def weibo_to_mongodb(self, wrote_count):
        ...
        print('{0}条微博写入MongoDB数据库完毕'.format(self.got_count))
    ...
    def mysql_create_database(self, mysql_config, sql):
        ...
        try:
            ...
            sys.exit('系统中可能没有安装pymysql库，请先运行 pip install pymysql ，再运行程序')
        try:
            ...
            sys.exit('系统中可能没有安装或正确配置MySQL数据库，请先根据系统环境安装或配置MySQL，再运行程序')
    ...
    def mysql_insert(self, mysql_config, table, data_list):
        ...
        if len(data_list) > 0:
            ...
            sql = """INSERT INTO weibo.{table}({keys}) VALUES ({values}) ON
    ...
    def weibo_to_mysql(self, wrote_count):
        ...
        # 创建'weibo'表
        create_table = """
                CREATE TABLE IF NOT EXISTS weibo.weibo (
                id varchar(20) NOT NULL,
        ...
        for w in info_list:
            ...
            weibo_list.append(w)
        # 在'weibo'表中插入或更新微博数据
        self.mysql_insert(self.mysql_config, 'weibo', retweet_list)
        self.mysql_insert(self.mysql_config, 'weibo', weibo_list)
        print('{0}条微博写入MySQL数据库完毕'.format(self.got_count))
    ...
    def update_user_config_file(self, user_config_file_path):
        ...
        with open(user_config_file_path, 'rb') as f:
            try:
                ...
                sys.exit('{0}文件应为utf-8编码，请先将文件编码转为utf-8再运行程序'.format(user_config_file_path))
    ...
    def get_pages(self):
        ...
        try:
            ...
            print('微博爬取完成，共爬取{0}条微博'.format(self.got_count))
        except Exception as e:
            print("Error: ", repr(e))
            traceback.print_exc()
    ...
    def get_user_config_list(self, file_path):
        ...
        with open(file_path, 'rb') as f:
            try:
                ...
                sys.exit('{0}文件应为utf-8编码，请先将文件编码转为utf-8再运行程序'.format(file_path))
            user_config_list = []
            for line in lines:
                info = line.split(' ')
                user_config = dict()
...
def get_config():
    ...
    if not os.path.isfile(config_path):
        sys.exit('当前路径：{0} 不存在配置文件config.json'.format((os.path.split(os.path.realpath(__file__))[0])))
    try:
        with open(config_path) as f:
            config = json.loads(f.read())
            return config
    except ValueError:
        sys.exit('config.json 格式不正确')
